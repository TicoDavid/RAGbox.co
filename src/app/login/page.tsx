...
export default function LoginPage() {
  ...
  const validateAndSignIn = async (provider: string) => {
  ...
  const handleGoogleAuth = () => validateAndSignIn('google')
  const handleMicrosoftAuth = () => validateAndSignIn('azure-ad')
    ...
    <main className="min-h-screen bg-void flex items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        ...
        <motion.div className="space-y-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <motion.button onClick={handleGoogleAuth} disabled={validating} className={cn('w-full flex items-center justify-center gap-3 px-6 py-4 rounded-full bg-white text-black font-medium transition-all duration-200 hover:bg-white/90 hover:shadow-lg active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed')} whileHover={{ scale: validating ? 1 : 1.02 }} whileTap={{ scale: 0.98 }}>
            ...
            <span>{validating ? 'Verifying code...' : 'Continue with Google'}</span>
          </motion.button>

          <motion.button onClick={handleMicrosoftAuth} disabled={validating} className={cn('w-full flex items-center justify-center gap-3 px-6 py-4 rounded-full bg-white text-black font-medium transition-all duration-200 hover:bg-white/90 hover:shadow-lg active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed')} whileHover={{ scale: validating ? 1 : 1.02 }} whileTap={{ scale: 0.98 }}>
            <svg className="w-5 h-5" viewBox="0 0 24 24">
            ...
            <span>{validating ? 'Verifying code...' : 'Continue with Microsoft'}</span>
          </motion.button>
        </motion.div>
