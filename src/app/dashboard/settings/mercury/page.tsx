  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-8">
      ...
      {emailToast && (
        <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-lg text-sm font-medium shadow-lg transition-all ${
          emailToast.includes('failed') || emailToast.includes('Failed') || emailToast.includes('error')
            ...
            : 'bg-[var(--success)]/90 text-[var(--text-primary)]'
        }`}>
          {emailToast}
        </div>
      )}
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Identity</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-default)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--brand-blue)]"
      ...
      <section className="space-y-4">
        ...
        {emailLoading ? (
          ...
          <div className="space-y-3 p-4 rounded-lg border border-[var(--border-default)] bg-[var(--bg-tertiary)]">
            <div className="flex items-center justify-between">
              ...
              {emailStatus.errorCount && emailStatus.errorCount > 0 ? (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-[var(--warning)]">{emailStatus.errorCount} error{emailStatus.errorCount > 1 ? 's' : ''}</span>
                </div>
              ) : null}
            </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-[var(--text-primary)] font-mono">{emailStatus.emailAddress}</span>
              </div>
              {emailStatus.lastRefreshed && (
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[var(--text-secondary)]">Last refreshed</span>
              ...
              {emailStatus.watchExpires && (
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[var(--text-secondary)]">Watch expires</span>
                  <span className="text-xs text-[var(--text-secondary)]">
              ...
              {emailStatus.lastError && (
                <div className="mt-2 p-2 rounded bg-[var(--danger)]/10 border border-[var(--danger)]/20">
                  <span className="text-xs text-[var(--danger)]">Last error: {emailStatus.lastError}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-[var(--border-default)]">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[var(--brand-blue)] hover:bg-[var(--brand-blue-hover)] text-[var(--text-primary)] text-xs font-medium transition-colors disabled:opacity-50"
              >
                {emailAction === 'testing' ? (
              ...
              {disconnectConfirm ? (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[var(--danger)]">Are you sure?</span>
                  <button
                    onClick={handleDisconnectGmail}
                    disabled={emailAction === 'disconnecting'}
                    className="px-3 py-1.5 rounded-md bg-[var(--danger)] hover:bg-[var(--danger)] text-[var(--text-primary)] text-xs font-medium transition-colors disabled:opacity-50"
                  >
                    {emailAction === 'disconnecting' ? 'Disconnecting...' : 'Yes, disconnect'}
                  </button>
                  <button
                    onClick={() => setDisconnectConfirm(false)}
                    className="px-3 py-1.5 rounded-md border border-[var(--border-default)] text-[var(--text-secondary)] text-xs hover:text-[var(--text-primary)] transition-colors"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[var(--danger)]/30 text-[var(--danger)] text-xs font-medium hover:bg-[var(--danger)]/10 transition-colors"
                >
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[var(--brand-blue)] hover:bg-[var(--brand-blue-hover)] text-[var(--text-primary)] text-sm font-medium transition-colors disabled:opacity-50"
          >
      ...
      <section className="space-y-4">
        ...
        <div>
          <label className="block text-xs text-[var(--text-secondary)] mb-1">Personality Prompt</label>
          <textarea
            value={personalityPrompt}
            onChange={(e) => { setPersonalityPrompt(e.target.value); setSelectedPreset('custom') }}
            rows={4}
            className="w-full px-3 py-2 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-default)] text-[var(--text-primary)] text-sm focus:outline-none focus:border-[var(--brand-blue)] resize-none"
      ...
      {/* Greeting */}
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Greeting Message</h2>
        <textarea
          value={greeting}
      ...
      {/* Email Signature */}
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Email Signature</h2>
        <textarea
        ...
        <div className="space-y-3">
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs text-[var(--text-secondary)]">High Confidence</label>
              <span className="text-xs text-[var(--text-primary)] font-mono">{Math.round(silenceHigh * 100)}%</span>
            </div>
            <input
              type="range"
              min="0.5"
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs text-[var(--text-secondary)]">Medium Confidence</label>
              <span className="text-xs text-[var(--text-primary)] font-mono">{Math.round(silenceMed * 100)}%</span>
            </div>
            <input
              type="range"
              min="0.3"
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-[var(--border-default)]">
        <button
          ...
          className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-[var(--brand-blue)] hover:bg-[var(--brand-blue-hover)] text-[var(--text-primary)] text-sm font-medium transition-colors disabled:opacity-50"
        >
