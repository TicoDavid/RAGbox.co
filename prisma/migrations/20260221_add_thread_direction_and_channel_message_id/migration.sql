...
-- Backfill direction for existing messages based on role
UPDATE mercury_thread_messages SET direction = 'outbound' WHERE role = 'assistant' AND direction = 'inbound';

-- Index for deduplication by external message ID
CREATE INDEX IF NOT EXISTS idx_thread_messages_channel_message_id
  ON mercury_thread_messages (channel_message_id) WHERE channel_message_id IS NOT NULL;
