...
import { RAGboxNode } from './nodes/ragbox_node';
...
export class InworldGraphWrapper {
  ...
  static async create(props: CreateGraphPropsInterface) {
    ...
    // RAGboxNode replaces DialogPromptBuilder + RemoteLLMChatNode.
    // It takes State, calls the Go backend /api/chat, parses SSE, and returns voice-ready text.
    const ragboxNode = new RAGboxNode({
      id: `ragbox-node${postfix}`,
    });
