...
export class InworldApp {
  ...
  async initialize() {
    ...
    // Initialize VAD client only if Assembly.AI is available (needed for audio pipeline)
    if (this.env.assemblyAIApiKey) {
      ...
      console.log('Skipping VAD model load (no Assembly.AI key — text-only mode)');
    }
    ...
    if (this.env.assemblyAIApiKey) {
      ...
      console.log('✓ Text-only mode (no audio STT)\n');
    }
  }
  ...
  async load(req: any, res: any) {
    ...
    if (!this.env.assemblyAIApiKey) {
      console.warn(`Assembly.AI STT requested but ASSEMBLY_AI_API_KEY not configured — session will use text-only mode`);
    }
