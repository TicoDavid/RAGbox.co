...
import { DashboardBridge } from './components/dashboard_bridge';
...
// ---------------------------------------------------------------------------
// Dashboard Voice Bridge (/agent/ws)
// Auto-creates sessions and translates between dashboard ↔ Inworld protocols
// ---------------------------------------------------------------------------

dashboardWS.on('connection', async (ws, request) => {
  const { query: q } = parse(request.url!, true);
  const config = {
    userId: q.userId?.toString() || 'anonymous',
    role: q.role?.toString() || 'User',
    privilegeMode: q.privilegeMode === 'true',
  };

  const bridge = new DashboardBridge(ws, inworldApp, config);

  try {
    await bridge.initialize();
  } catch (err) {
    console.error('[DashboardBridge] Init failed:', err);
  ...
  ws.on('close', (code, reason) => {
    console.log(
      `[DashboardBridge] WebSocket closed: code=${code}, reason=${reason.toString()}`,
    );
    bridge.cleanup();
  });
});
